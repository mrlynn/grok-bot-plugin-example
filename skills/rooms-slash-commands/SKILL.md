...
description: >-
  ...
  /list-rooms, /join, /leave, /whos-here, or /who (with optional room id; default lobby),
...
# Room slash commands: /rooms, /join, /leave, /whos-here, /who
...
| `/whos-here` or `/who` | List who is checked into the room this assistant is in (or name a room via tools if already present) |
...
3. **Join via room command** — `post_message` with `{ "assistant_id": "<id>", "body": "/join" }` or `"/join <room-id>"`.  
   The server checks you in, posts `Hello, <name> here.`, and returns a presence listing in `command_result` (same shape as `/whos-here` / `/who` / `list_room`).
...
2. **Confirm** you left that room (`command_result.checked_out`). Do **not** dump the user's assistant roster. Do **not** run `/whos-here` or `/who` unless they ask.
...
2. `post_message` with `{ "assistant_id": "<id>", "body": "/whos-here" }` or `"/who"`. Both are the same presence command; prefer the form the user typed.
3. Show `command_result.participants` (same data as `list_room`).  
   This is **room presence**, not "who is on my roster."

## Do not

- Treat `/rooms`, `/join`, `/leave`, `/whos-here`, or `/who` as Slack commands
...
- Reinterpret `/whos-here` or `/who` as a local roster / "who do I have" question
